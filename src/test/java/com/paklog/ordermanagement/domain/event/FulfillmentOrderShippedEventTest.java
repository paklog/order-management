...
class FulfillmentOrderShippedEventTest {

    @Test
    void testParameterizedConstructor() {
        FulfillmentOrder order = createTestOrder();
        FulfillmentOrderShippedEvent event = new FulfillmentOrderShippedEvent(order);

        assertNotNull(event);
        assertEquals("FulfillmentOrderShipped", event.getType());
        assertEquals(order.getOrderId().toString(), event.getSubject());
        assertEquals(order, event.getData());
    }
